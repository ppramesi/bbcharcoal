...
<script language="javascript">
  ...
  function setState(kind) {
    if($jq('#' + kind + '-country').val() == 'US' || $jq('#' + kind + '-country').val() == null) {
      ...
      <?php foreach(PHPurchaseCommon::getZones('US') as $code => $name): ?>
      ...
      $jq('#' + kind + '-state').empty(); 
      $jq('#' + kind + '-state').removeAttr('disabled');
      <?php foreach(PHPurchaseCommon::getZones('AU') as $code => $name): ?>
        $jq('#' + kind + '-state').append('<option value="<?php echo $code ?>"><?php echo $name ?></option>');
      <?php endforeach; ?>
    }
      ...
      $jq('#' + kind + '-state').empty(); 
      $jq('#' + kind + '-state').removeAttr('disabled');
      <?php foreach(PHPurchaseCommon::getZones('CA') as $code => $name): ?>
        $jq('#' + kind + '-state').append('<option value="<?php echo $code ?>"><?php echo $name ?></option>');
      <?php endforeach; ?>
    }
    else {
