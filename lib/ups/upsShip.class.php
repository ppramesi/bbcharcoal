...
class upsShip {
	...
	function buildRequestXML($shipmentArgs) {
		$xml = $this->ups->access();
		...
		$ShipmentConfirmRequestXML->push('ShipTo');
			$ShipmentConfirmRequestXML->element('CompanyName', $shipmentArgs['CompanyName']);
			$ShipmentConfirmRequestXML->element('AttentionName', $shipmentArgs['AttentionName']);
			$ShipmentConfirmRequestXML->push('PhoneNumber');
				$ShipmentConfirmRequestXML->push('StructuredPhoneNumber');
					$ShipmentConfirmRequestXML->element('PhoneDialPlanNumber', $shipmentArgs['PhoneAreaCode']);
					$ShipmentConfirmRequestXML->element('PhoneLineNumber', $shipmentArgs['PhoneNumber']);
					$ShipmentConfirmRequestXML->element('PhoneExtension', '');
				$ShipmentConfirmRequestXML->pop(); // end StrurcturedPhoneNumber 
			$ShipmentConfirmRequestXML->pop(); // end PhoneNumber 
			$ShipmentConfirmRequestXML->push('Address');
				$ShipmentConfirmRequestXML->element('AddressLine1', $shipmentArgs['AddressLine1']);
				$ShipmentConfirmRequestXML->element('City', $shipmentArgs['City']);
				$ShipmentConfirmRequestXML->element('StateProvinceCode', $shipmentArgs['StateProvinceCode']);
				$ShipmentConfirmRequestXML->element('CountryCode', $shipmentArgs['CountryCode']);
				$ShipmentConfirmRequestXML->element('PostalCode', $shipmentArgs['PostalCode']);
				$ShipmentConfirmRequestXML->element('ResidentialAddress', $shipmentArgs['ResidentialAddress']);
			$ShipmentConfirmRequestXML->pop(); // end Address
		$ShipmentConfirmRequestXML->pop(); // end ShipTo
		$ShipmentConfirmRequestXML->push('Service');
			$ShipmentConfirmRequestXML->element('Code', $shipmentArgs['ShippingCode']);
			$ShipmentConfirmRequestXML->element('Description', $shipmentArgs['ShippingDescription']);
		...
		$ShipmentConfirmRequestXML->push('PaymentInformation');
			$ShipmentConfirmRequestXML->push('Prepaid');
				$ShipmentConfirmRequestXML->push('BillShipper');
					...
					$ShipmentConfirmRequestXML->element('AccountNumber', UPS_SHIPPERNUMBER);
		...
		$ShipmentConfirmRequestXML->push('ShipmentServiceOptions');
			$ShipmentConfirmRequestXML->push('OnCallAir');
				$ShipmentConfirmRequestXML->push('PickupDetails');
					$ShipmentConfirmRequestXML->element('PickupDate', $shipmentArgs['PickupDate']);
		...
		foreach($shipmentArgs['Packages'] as $packageArg) {
			$ShipmentConfirmRequestXML->push('Package');
				$ShipmentConfirmRequestXML->push('PackagingType');
					$ShipmentConfirmRequestXML->element('Code', $packageArg['PackageTypeCode']);
				...
				$ShipmentConfirmRequestXML->push('Dimensions');
					$ShipmentConfirmRequestXML->push('UnitOfMeasurement');
						$ShipmentConfirmRequestXML->element('Code', $packageArg['PackageDimensionType']);
					$ShipmentConfirmRequestXML->pop(); // end UnitOfMeasurement
					$ShipmentConfirmRequestXML->element('Length', $packageArg['PackageLength']);
					$ShipmentConfirmRequestXML->element('Width', $packageArg['PackageWidth']);
					$ShipmentConfirmRequestXML->element('Height', $packageArg['PackageHeight']);
				$ShipmentConfirmRequestXML->pop(); // end Dimensions
				$ShipmentConfirmRequestXML->push('PackageWeight');
					$ShipmentConfirmRequestXML->element('Weight', $packageArg['PackageWeight']);
				$ShipmentConfirmRequestXML->pop(); // end PackageWeight
				$ShipmentConfirmRequestXML->push('ReferenceNumber');
					$ShipmentConfirmRequestXML->element('Code', $packageArg['ReferenceCode']);
					$ShipmentConfirmRequestXML->element('Value', $packageArg['ReferenceValue']);
				...
				$ShipmentConfirmRequestXML->push('PackageServiceOptions');
					$ShipmentConfirmRequestXML->push('InsuredValue');
						$ShipmentConfirmRequestXML->element('CurrencyCode', $packageArg['OrderCurrency']);
						$ShipmentConfirmRequestXML->element('MonetaryValue', $packageArg['OrderPrice']);
	...
	$ShipmentConfirmRequestXML->push('LabelSpecification');
		...
		$ShipmentConfirmRequestXML->element('HTTPUserAgent', $_SERVER['HTTP_USER_AGENT']);
...
$ShipmentConfirmRequestXML->pop(); // ShipmentConfirmReques
		...
		$xml->push('ShipmentAcceptRequest');
			$xml->push('Request');
				$xml->push('TransactionReference');
					$xml->element('CustomerContext', 'ShipmentAccept');
