...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['phpurchase-action'] == 'save promotion') {
...
<div class='wrap'>
  <p style='width: 400px;'>You may create promotion codes (coupon codes) to reduce the total cost 
    of your customer's purchase by either a specific dollar amount (i.e. $10) or a percentage (i.e. 10%). 
    You may also set a minimum order amount that must be reached before the promotion code may be 
    used. For example, you may create a promotion for 10% off all orders over $50.</p>
  <p style='width: 400px;'>NOTE: Promotion code discounts do not affect shipping costs.</p>
  
  <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method='post'>
    <input type='hidden' name='phpurchase-action' value='save promotion' />
    ...
    <ul>
      ...
      <li>
        ...
        <select name='promo[type]' id='promo_type'>
          <option value='dollar' <?php if($promo->type == 'dollar') { echo 'selected'; } ?>>Dollar Amount</option>
      ...
      <li>
        ...
        <?php if($promo->id > 0): ?>
          <a href='?page=phpurchase-promotions' class='button-secondary linkButton' style="">Cancel</a>
        <?php endif; ?>
      </li>
    </ul>
  ...
  <tbody>
    <?php foreach($promos as $p): ?>
     <tr>
       ...
       <td>
         <a href='?page=phpurchase-promotions&task=edit&id=<?php echo $p->id ?>'>Edit</a> | 
         <a class='delete' href='?page=phpurchase-promotions&task=delete&id=<?php echo $p->id ?>'>Delete</a>
       </td>
     </tr>
    <?php endforeach; ?>
  </tbody>
