...
$rate = new PHPurchaseShippingRate();
$product = new PHPurchaseProduct();
$setting = new PHPurchaseSetting();

if($_SERVER['REQUEST_METHOD'] == "POST") {
  if($_POST['phpurchase-action'] == 'save rule') {
    $rule->setData($_POST['rule']);
    $rule->save();
    $rule->clear();
  }
  elseif($_POST['phpurchase-action'] == 'save shipping method') {
    $method->setData($_POST['shipping_method']);
    $method->save();
    $method->clear();
  }
  elseif($_POST['phpurchase-action'] == 'save product rate') {
    $rate->setData($_POST['rate']);
    $rate->save();
    $rate->clear();
  }
  elseif($_POST['phpurchase-action'] == 'save ups account info') {
    ...
    if(is_array($methods)) {
      foreach($methods as $methodData) {
        ...
        $m = new PHPurchaseShippingMethod();
        $m->code = $code;
        $m->name = $name;
        $m->carrier = 'ups';
        $m->save();
  ...
  elseif($_POST['phpurchase-action'] == 'enable live rates') {
    $setting->key = 'use_live_rates';
    $setting->value = 1;
    $setting->save();
  }
  elseif($_POST['phpurchase-action'] == 'disable live rates') {
    $setting->key = 'use_live_rates';
    $setting->value = '';
    $setting->save();
  }
...
<div class='wrap'>
  ...
  <div style="padding: 10px; 25px; width: 600px; background-color: #EEE; border: 1px solid #CCC; -moz-border-radius: 5px; -webkit-border-radius: 5px;">
    ...
    <?php if($setting->lookupValue('use_live_rates')): ?>
      <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method='post'>
        ...
        <input type='hidden' name='phpurchase-action' value='disable live rates' />
    ...
    <?php else: ?>
      <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method='post'>
        ...
        <input type='hidden' name='phpurchase-action' value='enable live rates' />
  ...
  <?php if($setting->lookupValue('use_live_rates')): ?>
    ...
    <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method='post'>
      <input type='hidden' name='phpurchase-action' value='save ups account info' />
      <ul>
        <li>
          <label class="med">Username:</label>
          <input type='text' name='ups[ups_username]' id='ups_username' value='<?php echo $setting->lookupValue('ups_username'); ?>' />
        </li>
        <li>
          <label class="med">Password:</label>
          <input type='text' name='ups[ups_password]' id='ups_password' value='<?php echo $setting->lookupValue('ups_password'); ?>' />
        </li>
        <li>
          <label class="med">API Key:</label>
          <input type='text' name='ups[ups_apikey]' id='ups_apikey' value='<?php echo $setting->lookupValue('ups_apikey'); ?>' />
        </li>
        <li>
          <label class="med">Account number:</label>
          <input type='text' name='ups[ups_account]' id='ups_account' value='<?php echo $setting->lookupValue('ups_account'); ?>' />
        </li>
        <li>
          <label class="med">Ship from zip:</label>
          <input type='text' name='ups[ups_ship_from_zip]' id='ups_ship_from_zip' value='<?php echo $setting->lookupValue('ups_ship_from_zip'); ?>' />
        </li>
        <li>
        ...
        <li>
          <?php
            $services = PHPurchaseCommon::getUpsServices();
  ...
  <?php else: ?>
    ...
    <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method='post'>
      <input type='hidden' name='phpurchase-action' value='save shipping method' />
      ...
      <ul>
        ...
        <li>
          ...
          <?php if($method->id > 0): ?>
          <a href='?page=phpurchase-shipping' class='button-secondary linkButton' style="">Cancel</a>
          <?php endif; ?>
        </li>
      </ul>
    ...
    <tbody>
      <?php foreach($methods as $m): ?>
        <tr>
          ...
          <td>
           <a href='?page=phpurchase-shipping&task=edit_method&id=<?php echo $m->id ?>'>Edit</a> | 
           <a class='delete' href='?page=phpurchase-shipping&task=delete_method&id=<?php echo $m->id ?>'>Delete</a>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
    ...
    <?php if(count($method->getModels()) && count($product->getModels())): ?>
      <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method='post'>
        <input type="hidden" name="phpurchase-action" value="save product rate" />
    ...
    <tbody>
      <?php foreach($rates as $r): ?>
        ...
        <tr>
          ...
          <td>
           <a href='?page=phpurchase-shipping&task=edit_rate&id=<?php echo $r->id ?>'>Edit</a> | 
           <a class='delete' href='?page=phpurchase-shipping&task=delete_rate&id=<?php echo $r->id ?>'>Delete</a>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
    ...
    <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method='post'>
      <input type='hidden' name='phpurchase-action' value='save rule' />
        ...
        <tbody>
          <?php foreach($rules as $rule): ?>
           ...
           <tr>
             ...
             <td>
               <a href='?page=phpurchase-shipping&task=edit&id=<?php echo $rule->id ?>'>Edit</a> | 
               <a class='delete' href='?page=phpurchase-shipping&task=delete&id=<?php echo $rule->id ?>'>Delete</a>
             </td>
           </tr>
          <?php endforeach; ?>
        </tbody>
